...
export function getAntdTheme(mode: ThemeMode, role?: string | null): ThemeConfig {
  ...
  return {
    ...
    components: {
      ...
      // `colorBgContainer: transparent` prive Ant de la surface sur laquelle il
      // calcule ses états : sans `rowHoverBg` ni `headerSortHoverBg` explicites,
      // le survol d'une ligne retombait sur un noir opaque, illisible. On les
      // fixe donc sur la surface discrète du DS.
      Table: {
        colorBgContainer: 'transparent',
        headerBg: 'transparent',
        rowHoverBg: hsl(t.muted),
        headerSortHoverBg: hsl(t.muted),
        headerSortActiveBg: hsl(t.muted),
        headerFilterHoverBg: hsl(t.muted),
        // Colonne triée : sans ce jeton, Ant peignait la colonne entière en
        // noir opaque, corps compris.
        bodySortBg: hsl(t.muted),
        // Cellules figées : elles sont peintes à part et retomberaient sur la
        // surface transparente, « trouant » la ligne survolée.
        fixedHeaderSortActiveBg: hsl(t.muted),
      },
