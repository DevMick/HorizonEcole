...
import EstablishmentProfilePage from './pages/EstablishmentProfilePage';
...
import PrimaryClassesPage from './pages/primary/PrimaryClassesPage';
import PrimaryEvaluationsPage from './pages/primary/PrimaryEvaluationsPage';
import PrimaryGradeEntryPage from './pages/primary/PrimaryGradeEntryPage';
import PrimaryMyClassPage from './pages/primary/PrimaryMyClassPage';
import PrimaryMyStudentsPage from './pages/primary/PrimaryMyStudentsPage';
import PrimaryAnnualReportPage from './pages/primary/PrimaryAnnualReportPage';
import PrimaryTeacherProfilePage from './pages/primary/PrimaryTeacherProfilePage';
...
function App() {
  ...
  // Le compte est conservé en localStorage entre deux visites, menus compris.
  // On le relit au démarrage : sans cela, un changement de droits — nouveau
  // menu, rôle resynchronisé sur le type d'établissement — ne se verrait qu'à
  // la prochaine connexion, et l'utilisateur resterait devant un sidebar
  // périmé sans savoir qu'il doit se déconnecter.
  useEffect(() => {
    if (!isAuthenticated || !localStorage.getItem('token')) return;
    authApi
      .getProfile()
      .then(({ data }) => {
        if (data?.data) useAuthStore.getState().updateUser(data.data);
      })
      .catch(() => {
        // Un 401 est déjà traité par l'intercepteur ; le reste (API arrêtée,
        // réseau) ne doit pas empêcher l'application de s'afficher.
      });
  }, [isAuthenticated]);
  ...
  if (!hasValidAuth) {
    return (
      ...
      <Routes>
        ...
        {/* Création d'établissement — publique par nécessité : c'est par elle
            qu'une école entre dans l'application, avant qu'aucun compte
            n'existe. */}
        <Route path="/creer-etablissement" element={<CreateEstablishmentPage />} />
      ...
      <AntApp>
        <Layout>
          <Routes>
            ...
            <Route path="/parent/bulletins" element={<Navigate to="/parent/grades" replace />} />
  ...
  return (
    <AntApp>
      <Layout>
        ...
        <Route path="/etablissement" element={<EstablishmentProfilePage />} />
        ...
        {/* École primaire (CP1 → CM2) — module au calcul de moyenne propre.
            Les mêmes routes servent l'administration et l'enseignant titulaire :
            les pages adaptent leur portée au rôle plutôt que de se dédoubler. */}
        <Route path="/primary/classes" element={<PrimaryClassesPage />} />
        <Route path="/primary/evaluations" element={<PrimaryEvaluationsPage />} />
        <Route path="/primary/grades" element={<PrimaryGradeEntryPage />} />
        <Route path="/primary/my-class" element={<PrimaryMyClassPage />} />

        {/* Espace du titulaire du primaire — écrans en lecture, sauf la saisie
            des notes : l'inscription, le calendrier et la grille relèvent de
            l'administration, lui vient y travailler sa classe. */}
        <Route path="/primary/my-students" element={<PrimaryMyStudentsPage />} />
        <Route path="/primary/annual-report" element={<PrimaryAnnualReportPage />} />
        <Route path="/primary/profile" element={<PrimaryTeacherProfilePage />} />
        <Route path="/primary/saisie" element={<PrimarySaisieNotesPage />} />

        {/* Finance */}
        <Route path="/finance/payment-conditions" element={<PaymentConditionsPage />} />
