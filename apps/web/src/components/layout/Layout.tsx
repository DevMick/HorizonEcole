...
export default function Layout({ children }: LayoutProps) {
  ...
  // École primaire pure : les raccourcis mobiles du titulaire ne sont pas ceux
  // d'un professeur du secondaire (cf. getTabbarItems).
  const isPrimaryOnly = establishment?.schoolType === 'PRIMAIRE';

  const { menuItems, selectedKey } = useAppNavigation();
  const sections = useMemo(() => adaptMenu(menuItems), [menuItems]);
  const tabbarItems = useMemo(
    () => getTabbarItems(user?.role, { primaryOnly: isPrimaryOnly }),
    [user?.role, isPrimaryOnly],
  );
