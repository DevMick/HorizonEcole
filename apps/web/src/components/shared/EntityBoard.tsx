...
export interface EntityCardConfig {
  ...
  /** Retire le bouton Modifier de cette carte (ex. entité non modifiable). */
  hideEdit?: boolean;
  /** Retire le bouton Supprimer de cette carte (ex. entité non supprimable). */
  hideDelete?: boolean;
}
...
export interface EntityBoardProps {
  ...
  onSecondary?: () => void;
  /**
   * Recherche : optionnelle. Une liste courte et figée — les six classes du
   * primaire, par exemple — n'a rien à filtrer. Omettre `onSearchChange` retire
   * le champ, et la barre d'outils disparaît s'il ne reste aucun filtre.
   */
  search?: string;
...
export function EntityBoard(props: EntityBoardProps) {
  const {
    ...
    filters, items, loading, cardOf, onEdit, onDelete, cardActions, deleteLabel, emptyTitle, emptyText, pageSize = 12, columns,
  } = props;
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      {loading ? (
        ...
        <>
          <div className="ds-entity-grid" style={columns !== undefined ? { gridTemplateColumns: `repeat(auto-fit, minmax(min(${Math.floor(980 / columns)}px, 100%), 1fr))` } : undefined}>
            {paged.map((item) => {
              const c = cardOf(item);
              return (
                <Card key={c.key} accent hover={!!c.onClick} className={cn('ds-entity-card', c.onClick && 'cursor-pointer')} onClick={c.onClick}>
                  ...
                  {((onEdit && !c.hideEdit) || (onDelete && !c.hideDelete) || cardActions) && (
                    <span className="ds-entity-actions" onClick={(e) => e.stopPropagation()}>
                      {cardActions?.(item)}
                      {onEdit && !c.hideEdit && <Button variant="ghost" size="sm" iconOnly icon={<Pencil aria-hidden />} aria-label="Modifier" onClick={() => onEdit(item)} />}
                      {onDelete && !c.hideDelete && (
                        <Button
                          variant="ghost"
                          size="sm"
                          iconOnly
