...
export interface TeacherFormPageProps {
  ...
  /** true si l'établissement est une école primaire */
  isPrimary?: boolean;
  /** Classes disponibles pour l'affectation (primaire uniquement) :
   *  classes sans titulaire + éventuellement la classe déjà affectée à cet enseignant */
  availableClasses?: { id: string; name: string; level?: string | null }[];
  fileList: any[];
 ...
 *
 * Pour les établissements primaires, le champ « Matières » est remplacé par
 * « Classe affectée » : un enseignant du primaire est titulaire d'une seule
 * classe, et seules les classes sans titulaire (pour l'année en cours) sont
 * proposées — une classe déjà prise ne réapparaît pas dans la liste.
 */
export function TeacherFormPage(props: TeacherFormPageProps) {
  const {
    form, editing, subjects, isPrimary, availableClasses,
    fileList, onFileListChange, attachmentsToRemove, onRemoveAttachment,
      ...
      <Card>
        <Form form={form} layout="vertical" preserve={false}>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              {isPrimary ? (
                /* École primaire : affectation à une classe (titulaire) */
                <Form.Item
                  name="classId"
                  label="Classe affectée"
                  rules={[{ required: true, message: 'Veuillez affecter une classe à cet enseignant' }]}
                >
                  <Select
                    ...
                    notFoundContent={
                      <span className="text-ds-text-secondary text-sm">
                        Toutes les classes ont déjà un titulaire
                      </span>
                    }
                  />
                </Form.Item>
              ) : (
                /* Collège / Lycée : sélection des matières enseignées */
