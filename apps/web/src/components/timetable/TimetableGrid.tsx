...
export interface TimetableGridProps {
  ...
  readOnly?: boolean;
  /** Mode école primaire : pas de salle ni d'enseignant par créneau. */
  isPrimary?: boolean;
}
 ...
 */
export function TimetableGrid({ slots, days, cellFor, onDelete, loading, readOnly, isPrimary }: TimetableGridProps) {
                  ...
                  return (
                    <td key={d.value} className="ds-tt-td">
                      <div className={readOnly ? 'ds-tt-cell ds-tt-cell-ro' : 'ds-tt-cell'}>
                        ...
                        <span className="ds-tt-subject">{cell.subject?.name || cell.subject?.code || 'Activité'}</span>
                        {!isPrimary && !isEPS(cell) && cell.classroom?.name && <span className="ds-tt-room">{cell.classroom.name}</span>}
                        {!isPrimary && <span className="ds-tt-teacher">{cell.teacher ? `${cell.teacher.first_name ?? ''} ${cell.teacher.last_name ?? ''}`.trim() : 'Enseignant'}</span>}
                      </div>
                    </td>
                  );
                }
