...
export default function ParentHomePage() {
  ...
  const { data: grades } = useQuery({
    ...
    queryFn: async () =>
      (await api.get(`/parent/children/${childId}/grades?academicYearId=${currentYear.id}`)).data
        .data,
    enabled: !!childId && !!currentYear?.id && !isPrimary,
  });

  const { data: primaryData } = useQuery({
    queryKey: ['parent-primary', childId, currentYear?.id],
    queryFn: async () =>
      (await api.get(`/parent/children/${childId}/primary?academicYearId=${currentYear.id}`)).data
        .data,
    enabled: !!childId && !!currentYear?.id && isPrimary,
  });
  ...
  // Pour le primaire : les 3 dernières compositions publiées, triées par date.
  const recentCompositions = useMemo(() => {
    return [...(primaryData?.evaluations || [])]
      .sort(
        (a: any, b: any) =>
          new Date(b.evaluation.date).getTime() - new Date(a.evaluation.date).getTime(),
      )
      .slice(0, 3);
  }, [primaryData]);
  ...
  return (
    <div className="animate-fade-in mx-auto max-w-5xl">
      ...
      <div className="mb-4 grid gap-4 lg:grid-cols-2">
        ...
        {/* Dernières notes / compositions — ce que le parent vient chercher en premier */}
        <Card className="lg:col-span-2">
          <div className="ds-activity-head">
            <span className="flex items-center gap-2">
              ...
              />
              <strong className="font-display text-ds-text">
                {isPrimary ? 'Dernières compositions' : 'Dernières notes'}
              </strong>
            </span>
          ...
          {isPrimary ? (
            !primaryData ? (
              ...
              <p className="py-6 text-center text-sm text-ds-text-tertiary">
                Aucune composition publiée pour cette année.
              </p>
            ) : (
              <ul className="ds-course-list">
                {recentCompositions.map((r: any) => {
                  const avg = r.result?.average;
                  const scale = r.evaluation?.averageScale ?? 20;
                  const pass = avg !== null && avg !== undefined && avg >= scale / 2;
                  return (
                    <li key={r.evaluation.id} className="ds-course-item">
                      <span className="ds-course-time">
                        {dayjs(r.evaluation.date).format('DD/MM')}
                        <span>{r.evaluation.isExam ? 'Examen' : 'Composition'}</span>
                      </span>
                      <span className="ds-course-main">
                        <strong>{r.evaluation.name}</strong>
                        <span>{r.result?.mention || ''}</span>
                      </span>
                      {avg !== null && avg !== undefined ? (
                        ...
                        >
                          {Number(avg).toFixed(2).replace('.', ',')}
                          <span className="ml-0.5 text-[.72rem] font-medium text-ds-text-tertiary">
                            /{scale}
                          </span>
                        </span>
                      ) : (
                        <span className="ds-grade-avg shrink-0">—</span>
                      )}
                    </li>
                  );
                })}
