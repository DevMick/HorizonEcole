...
const createStudentSchema = z.object({
  body: z.object({
    // Facultatif : au primaire l'école n'attribue pas de matricule, l'API en
    // génère un (cf. StudentService.createStudent).
...
router.post('/:id/create-account', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    ...
    if (message.includes('non trouvé') || message.includes('déjà') || message.includes('existe déjà') || message.includes('pas de compte')) {
