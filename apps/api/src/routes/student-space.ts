...
async function currentStudent(req: AuthRequest, res: Response): Promise<SchoolChild | null> {
  ...
  return {
    ...
    cycle: student.class?.cycle ?? null,
  };
}
...
// ---------------------------------------------------------------------------
// GET /api/student/primary?academicYearId=
//
// Résultats du primaire, pour les élèves d'une classe CP1 → CM2 : compositions
// publiées, avec notes, moyenne, rang et décision.
// ---------------------------------------------------------------------------
router.get('/primary', async (req: AuthRequest, res, next) => {
  try {
    const student = await currentStudent(req, res);
    if (!student) return;
    ...
    res.json({ success: true, data: await getPrimaryResults(student, academicYearId) });
  } catch (error) {
    next(error);
  }
});
