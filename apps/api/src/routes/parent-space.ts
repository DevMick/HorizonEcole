...
async function resolveChild(
  ...
  return {
    ...
    cycle: link.student.class?.cycle ?? null,
  };
}
...
// ---------------------------------------------------------------------------
// GET /api/parent/children/:studentId/primary?academicYearId=
//
// Résultats du primaire : une composition = notes, moyenne, rang et décision.
// Seules les compositions publiées par l'administration remontent ici.
// ---------------------------------------------------------------------------
router.get('/children/:studentId/primary', async (req: AuthRequest, res, next) => {
  try {
    const child = await resolveChild(req, res, req.params.studentId);
    if (!child) return;
    ...
    res.json({ success: true, data: await getPrimaryResults(child, academicYearId) });
  } catch (error) {
    next(error);
  }
});
