...
import { z } from 'zod';
import { CustomPaymentPlanService, createCustomPaymentPlanSchema, updateCustomPaymentPlanSchema, upsertLevelTemplateSchema } from '../services/customPaymentPlan.service';
...
// Get the payment-schedule template for a level/academic year
router.get(
  '/template/level',
  authenticate,
  requireRole('ADMIN', 'ACCOUNTANT'),
  async (req, res, next) => {
    try {
      const { level, academicYearId, isForStateAssigned } = req.query;
      ...
      const template = await CustomPaymentPlanService.getLevelTemplate(level, academicYearId, isForStateAssigned === 'true');

      if (!template) {
        return res.status(404).json({
          success: false,
          error: 'No payment schedule template found for this level/academic year',
        });
      }
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const parsed = upsertLevelTemplateSchema.parse(req.body);
      const template = await CustomPaymentPlanService.upsertLevelTemplate(parsed, userId);

      res.json({
        success: true,
        data: template,
        message: 'Payment schedule template saved successfully',
      });
    } catch (error: any) {
