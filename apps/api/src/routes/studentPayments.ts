  ...
  async (req, res, next) => {
    try {
      ...
      if (error.code === 'P2002') {
        return res.status(400).json({
          success: false,
          error: 'Payment already exists for this installment',
        });
      }
      next(error);
    }
...
// Record a free-form advance payment (not tied to one specific échéance) —
// automatically allocated across the student's unpaid dues, oldest first.
router.post(
  '/advance',
  authenticate,
  requireRole('ADMIN', 'ACCOUNTANT'),
  ...
  async (req, res, next) => {
    try {
      const userId = (req as any).user.userId;
      const payments = await StudentPaymentService.recordAdvancePayment(req.body, userId);
