...
import { menuKeysForEstablishment, syncProtectedRoleMenus } from '../services/role-sync.service';
 ...
 */
async function allowedMenuKeys(req: AuthRequest, menuKeys: string[]): Promise<string[]> {
  const allowed = new Set(await menuKeysForEstablishment(req.user!.establishmentId));
  return menuKeys.filter((key) => allowed.has(key));
}
...
router.patch('/:id', authenticate, requireRole('ADMIN'), validate(updateRoleSchema), async (req: AuthRequest, res) => {
  try {
    ...
    const menuKeys =
      req.body.menuKeys === undefined ? undefined : await allowedMenuKeys(req, req.body.menuKeys);
