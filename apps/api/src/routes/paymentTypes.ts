...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const paymentTypes = await PaymentTypeService.getAllPaymentTypes(level);
    res.json({
      success: true,
      data: paymentTypes,
    });
    ...
    res.status(500).json({ message: 'Erreur lors de la récupération des types de versement' });
  }
});
...
// Create payment type for a level (Admin/Accountant only)
router.post('/', requireRole('ADMIN', 'ACCOUNTANT'), async (req: Request, res: Response) => {
  try {
    ...
    if (!level) {
      return res.status(400).json({ message: 'level est requis' });
    }

    const exists = await PaymentTypeService.existsByName(name, level);
    if (exists) {
      return res.status(400).json({ message: 'Un type de versement avec ce nom existe déjà pour ce niveau' });
    }

    const paymentType = await PaymentTypeService.createPaymentType({ name, level });
    res.status(201).json({
      success: true,
      data: paymentType,
    });
...
router.put('/:id', requireRole('ADMIN', 'ACCOUNTANT'), async (req: Request, res: Response) => {
  try {
    ...
    const existing = await PaymentTypeService.getPaymentTypeById(id);
    if (!existing) {
      return res.status(404).json({ message: 'Type de versement non trouvé' });
    }

    const nameExists = await PaymentTypeService.existsByName(name, existing.level, id);
    if (nameExists) {
      return res.status(400).json({ message: 'Un type de versement avec ce nom existe déjà pour ce niveau' });
    }
