...
import customPaymentPlansRoutes from './routes/customPaymentPlans';
import studentPaymentsRoutes from './routes/studentPayments';
import invoicesRoutes from './routes/invoices';
import primaryRoutes from './routes/primary';
...
app.use(cors({
  ...
  // Sans cela le navigateur masque l'en-tête aux appels `fetch`/axios : les
  // téléchargements (fiche de classement Word, exports) perdent le nom de
  // fichier proposé par le serveur et retombent sur un nom générique.
...
// Établissements — la création est publique (c'est par elle qu'une école entre
// dans l'application, avant tout compte) ; les autres routes du module sont
// authentifiées individuellement. Monté ici, avant les routes métier, pour la
// même raison que l'authentification.
...
app.use('/api/school-fee-rates', schoolFeeRatesRoutes);
app.use('/api/payment-types', paymentTypesRoutes);
app.use('/api/payment-conditions', paymentConditionsRoutes);
app.use('/api/custom-payment-plans', customPaymentPlansRoutes);
app.use('/api/student-payments', studentPaymentsRoutes);
app.use('/api/invoices', invoicesRoutes);
// Cycle primaire (CP1 → CM2) : grille, compositions, notes et résultats.
...
app.use('/uploads', (req, res, next) => {
  ...
  // helmet pose par défaut Cross-Origin-Resource-Policy: same-origin, qui empêche
  // d'embarquer ces fichiers via <img> depuis le front (origine différente en dev,
  // ex. le logo de l'établissement). On l'ouvre pour ce seul chemin.
