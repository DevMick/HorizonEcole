...
const subdirs = ['avatars', 'documents', 'students', 'parents', 'justificatifs', 'logos'];
...
// Logo de l'établissement : une seule image, stockée dans son propre dossier.
export const logoUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req: Request, file: Express.Multer.File, cb) => {
      cb(null, generateFileName('logos', file.originalname));
    },
  }),
  fileFilter: fileFilter(allowedMimeTypes.images),
  limits: {
    fileSize: fileSizeLimits.avatar, // 2MB
    files: 1,
  },
});
...
type UploadType = 'avatar' | 'document' | 'justificatif' | 'logo';

const subdirForType = (type: UploadType): string => {
  if (type === 'avatar') return 'avatars';
  if (type === 'justificatif') return 'justificatifs';
  if (type === 'logo') return 'logos';
  return 'documents';
};

export const getFileUrl = (filename: string, type: UploadType = 'document'): string => {
  return `/uploads/${subdirForType(type)}/${filename}`;
};

export const getFilePath = (filename: string, type: UploadType = 'document'): string => {
  return path.join(uploadDir, subdirForType(type), filename);
};
