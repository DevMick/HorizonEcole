...
import { getStudentYearResults } from './primary/primary-results.service';
...
export interface SchoolChild {
  ...
  /**
   * Cycle de la classe. Il décide de la façon de lire la scolarité : au
   * secondaire, des notes par matière agrégées par coefficient ; au primaire,
   * des compositions dont la moyenne est une somme divisée. Les deux espaces
   * s'appuient dessus pour appeler la bonne lecture.
   */
...
// ---------------------------------------------------------------------------
// Résultats du primaire — compositions publiées
//
// L'équivalent, pour le cycle primaire, de `getGrades` et `getBulletins`
// réunis : au primaire il n'y a pas de notes courantes d'un côté et un bulletin
// de l'autre, mais des compositions, dont chacune produit à la fois les notes,
// la moyenne, le rang et la décision. Une seule lecture suffit donc.
//
// ---------------------------------------------------------------------------
export async function getPrimaryResults(child: SchoolChild, academicYearId: string) {
  if (!child.classId || child.cycle !== 'PRIMAIRE') {
    return { class: null, evaluations: [], yearAverage: null, student: null };
  }

  return getStudentYearResults(child.id, academicYearId, { publishedOnly: false });
}
