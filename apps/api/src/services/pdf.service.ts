...
export class PDFService {
  ...
  private static invoicesDir = path.join(process.cwd(), 'uploads', 'invoices');
...
<body>
  ...
  static async generatePaymentReceipt(paymentId: string): Promise<string> {
    ...
    // A payment plan always exists for an invoiced student (see
    // InvoiceService.generateForInscription) — its total is the source of
    // truth for what's expected across the academic year.
    ...
    const totalExpectedAmount = Number(customPaymentPlan?.total_amount) || 0;
...
<body>
  ...
  static async generateStudentPaymentReceiptPDFDirect(paymentId: string): Promise<string> {
    ...
    // Source of truth for what's expected overall — a payment plan (échéancier)
    // always exists for an invoiced student (see InvoiceService.generateForInscription).
    const { StudentPaymentService } = await import('./studentPayment.service');
    const paymentStatus = await StudentPaymentService.getStudentPaymentStatus(
      payment.student_id,
      payment.academic_year_id
    );
    const totalExpectedAmount = Number(paymentStatus.summary.totalExpected);
...
<body>
  <div class="container">
    ...
    <div class="section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          <div class="info-label">Versement</div>
          <div class="info-value">Versement ${payment.custom_payment_plan_installments?.installment_number ?? ''}</div>
        </div>
        <div class="info-item">
    ...
    <div class="section">
      ...
      <table class="amount-table">
        ...
        <tbody>
          <tr>
            <td class="label">Montant total de l'échéancier</td>
            <td class="amount">${formatCurrency(totalExpectedAmount)}</td>
          </tr>
          <tr>
            <td class="label">Montant payé (cumul)</td>
  ...
  /**
   * Generate the invoice (facture) PDF listing what a student owes for an
   * academic year: fee breakdown + due dates. Distinct from the payment
   * receipt above, which documents money already received.
   */
  static async generateInvoicePDF(invoiceId: string): Promise<string> {
    ...
    const invoice = await prisma.invoices.findUnique({
      ...
      include: {
        ...
        lines: {
          include: { payment_types: { select: { name: true } } },
        },
        customPaymentPlan: {
          include: {
            custom_payment_plan_installments: {
    ...
    const fileName = `invoice-${invoiceId}-${invoice.invoice_number.replace(/[^a-zA-Z0-9]/g, '-')}.pdf`;
    ...
    // Contenu de la facture (ce qui est dû) — indépendant de l'échéancier (quand c'est dû).
    const linesHtml = invoice.lines
      .map(
        line => `
          <tr>
            <td class="label">${line.description}</td>
            <td class="amount">${formatCurrency(line.amount)}</td>
          </tr>`
      )
      .join('');

    const installments = invoice.customPaymentPlan?.custom_payment_plan_installments || [];
    const installmentsHtml = installments
      .map(
        inst => `
          <tr>
            <td class="label">Versement ${inst.installment_number}${inst.notes ? ` (${inst.notes})` : ''}</td>
            <td>${new Date(inst.due_date).toLocaleDateString('fr-FR')}</td>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Arial', 'Helvetica', sans-serif; font-size: 10px; line-height: 1.3; color: #333; padding: 8px; background: #fff; }
    .container { max-width: 100%; margin: 0 auto; background: #fff; border: 2px solid #228B22; border-radius: 6px; padding: 12px; }
    .header { text-align: center; border-bottom: 2px solid #228B22; padding-bottom: 6px; margin-bottom: 10px; }
    .header h1 { color: #228B22; font-size: 16px; font-weight: bold; margin-bottom: 3px; }
    .header h2 { color: #666; font-size: 12px; font-weight: normal; }
    .invoice-number { text-align: right; margin-bottom: 8px; font-weight: bold; color: #228B22; font-size: 10px; }
    .section { margin-bottom: 10px; }
    .section-title { background: #228B22; color: #fff; padding: 4px 8px; font-weight: bold; font-size: 10px; margin-bottom: 6px; border-radius: 3px; }
    .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; margin-bottom: 6px; }
    .info-item { display: flex; flex-direction: column; }
    .info-label { font-weight: bold; color: #666; font-size: 8px; margin-bottom: 2px; }
    .info-value { color: #333; font-size: 9px; }
    .amount-table { width: 100%; border-collapse: collapse; margin-top: 5px; font-size: 9px; }
    .amount-table th { background: #f5f5f5; padding: 5px 6px; text-align: left; font-weight: bold; font-size: 8px; border: 1px solid #ddd; }
    .amount-table td { padding: 5px 6px; border: 1px solid #ddd; font-size: 9px; }
    .amount-table .label { font-weight: 600; color: #333; }
    .amount-table .amount { text-align: right; font-weight: bold; color: #228B22; }
    .total-section { background: #f9f9f9; padding: 8px; border-radius: 4px; margin-top: 8px; border: 2px solid #228B22; }
    .total-row { display: flex; justify-content: space-between; align-items: center; padding: 4px 0; font-size: 11px; }
    .total-label { font-weight: bold; color: #333; }
    .total-amount { font-weight: bold; font-size: 14px; color: #228B22; }
    .footer { margin-top: 12px; padding-top: 8px; border-top: 1px solid #ddd; text-align: center; font-size: 7px; color: #666; }
    .status-cancelled { color: #ff4d4f; font-weight: bold; text-align: center; margin-bottom: 8px; }
  </style>
</head>
<body>
  <div class="container">
    ...
    ${invoice.status === 'CANCELLED' ? '<div class="status-cancelled">FACTURE ANNULÉE</div>' : ''}
    <div class="invoice-number">N° Facture: ${invoice.invoice_number}</div>
    ...
    <div class="section">
      ...
      <div class="info-grid">
        <div class="info-item">
          <div class="info-label">Matricule</div>
          <div class="info-value">${invoice.student.studentNumber}</div>
        </div>
        <div class="info-item">
          <div class="info-label">Nom complet</div>
          <div class="info-value">${invoice.student.lastName} ${invoice.student.firstName}</div>
        </div>
        <div class="info-item">
          <div class="info-label">Classe</div>
          <div class="info-value">${invoice.class.name}</div>
        </div>
        <div class="info-item">
          <div class="info-label">Année académique</div>
          <div class="info-value">${invoice.academicYear.name}</div>
        </div>
        <div class="info-item">
          <div class="info-label">Date d'émission</div>
          <div class="info-value">${new Date(invoice.issued_at).toLocaleDateString('fr-FR')}</div>
        </div>
        <div class="info-item">
          <div class="info-label">Émise par</div>
          <div class="info-value">${invoice.creator ? `${invoice.creator.firstName} ${invoice.creator.lastName}` : 'Système'}</div>
        </div>
      </div>
    </div>
    ...
    <div class="section">
      <div class="section-title">ÉCHÉANCIER DE PAIEMENT</div>
      <table class="amount-table">
        <thead>
          <tr>
            <th>Désignation</th>
    ...
    <div class="total-section">
      <div class="total-row">
        <span class="total-label">Montant total dû</span>
        <span class="total-amount">${formatCurrency(invoice.total_amount)} FCFA</span>
      </div>
    </div>

    <div class="footer">
      <p>Document généré le ${new Date().toLocaleDateString('fr-FR')} à ${new Date().toLocaleTimeString('fr-FR')} - Ce document liste les frais dus, il ne vaut pas reçu de paiement.</p>
    </div>
  </div>
</body>
</html>
